...
prepare_next_build_env_file() {
  ...
  if [[ -f "$NEXT_BUILD_ENV_FILE" ]]; then
    mv "$NEXT_BUILD_ENV_FILE" "$NEXT_BUILD_ENV_BACKUP_FILE"
  else
    (umask 077 && printf '%s\n' "generated by mc-aws deployment" >"$NEXT_BUILD_ENV_MARKER_FILE")
  fi
...
recover_interrupted_next_build_env_file() {
  if [[ -f "$NEXT_BUILD_ENV_BACKUP_FILE" ]]; then
    rm -f "$NEXT_BUILD_ENV_FILE"
    mv "$NEXT_BUILD_ENV_BACKUP_FILE" "$NEXT_BUILD_ENV_FILE"
  elif [[ -f "$NEXT_BUILD_ENV_MARKER_FILE" ]]; then
    rm -f "$NEXT_BUILD_ENV_FILE"
  fi
  rm -f "$NEXT_BUILD_ENV_MARKER_FILE"
}
...
mkdir -p .local-artifacts/cloudflare-tmp
...
if ! env -u AWS_ACCESS_KEY_ID -u AWS_SECRET_ACCESS_KEY -u AWS_SESSION_TOKEN \
  TMPDIR="$PWD/.local-artifacts/opennext-tmp" MC_BACKEND_MODE=aws ENABLE_DEV_LOGIN= pnpm exec opennextjs-cloudflare build --skipNextBuild --config "$WRANGLER_DEPLOY_CONFIG_FILE"; then
  echo ""
