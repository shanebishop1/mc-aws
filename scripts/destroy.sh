...
quiesce_instance_for_snapshot() {
  ...
  local stop_script parameters send_json command_id invocation_json refreshed_line refreshed_state refreshed_root_device refreshed_root_volume
  ...
  case "$initial_state" in
    running|pending)
      ...
      stop_script='set -eu; systemctl stop minecraft.service; i=0; while [ "$i" -lt 24 ]; do if ! systemctl is-active --quiet minecraft.service; then exit 0; fi; i=$((i+1)); sleep 5; done; exit 1'
...
# Backups and storage are report-only. This script intentionally has no EC2 delete-volume/delete-snapshot calls.
...
if [[ "$STACK_OWNED" == "true" && "$STACK_LIVE" == "1" ]]; then
  ...
  if ! assert_aws_account_now || ! assert_exact_stack_live_now || ! assert_runtime_iam_tags_now; then
    error "AWS stack/runtime IAM identity changed before final data preservation; blocking stack deletion"
    exit 1
  fi
  preserve_final_root_data || exit 1
  assert_manifest_unchanged
  if ! assert_aws_account_now || ! assert_exact_stack_live_now || ! assert_runtime_iam_tags_now; then
    error "AWS stack/runtime IAM identity changed after snapshot wait; blocking stack deletion"
    exit 1
  fi
  delete_stack_error=""
