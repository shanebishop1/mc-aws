...
export async function POST(request: NextRequest, context: Context): Promise<NextResponse> {
  ...
  const throttled = await enforceAgentRateLimit(
    request,
    auth.actorId,
    "/api/agent/sessions/[sessionId]/approvals/[approvalId]/revoke",
    true
  );
  ...
  try {
    ...
    return agentSuccess(
      await service.revokeApproval(auth.actorId, parseAgentId(sessionId), parseAgentId(approvalId), input)
    );
  } catch (error) {
