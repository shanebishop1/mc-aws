  ...
  return (
    <div className={`flex flex-col items-center justify-center space-y-12 ${className}`}>
      ...
      <div className="text-center space-y-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-6">
          {ip && <span className="font-sans text-xs tracking-[0.2em] text-luxury-black/50 uppercase">{ip}</span>}
        </motion.div>
      </div>
    </div>
  );
