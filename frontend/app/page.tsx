  ...
  return (
    <main className="min-h-screen flex flex-col items-center py-24 px-4 bg-luxury-cream selection:bg-luxury-green selection:text-white">
      ...
      <div className="flex-1 w-full max-w-4xl flex flex-col items-center justify-between min-h-[400px]">
        ...
        {/* Status Section */}
        <section className="flex-1 flex flex-col justify-center w-full mb-20">
        ...
        <section className="w-full grid grid-cols-1 md:grid-cols-3 gap-8 items-center justify-items-center">
            ...
            <div className="flex flex-col gap-6 w-full max-w-[200px] text-center md:text-right">
                <LuxuryButton variant="text" onClick={() => handleAction("Backup", "/api/backup")} disabled={status !== "running"}>
                    Backup
                </LuxuryButton>
                <LuxuryButton variant="text" onClick={() => handleAction("Hibernate", "/api/hibernate")} disabled={status === "hibernated"}>
                    Hibernate
                </LuxuryButton>
            </div>
            ...
            <div className="order-first md:order-none mb-8 md:mb-0">
                {status === "running" ? (
                    <LuxuryButton onClick={() => handleAction("Stop", "/api/stop")}>
                        Stop Server
                    </LuxuryButton>
                ) : (
                    <LuxuryButton onClick={() => handleAction("Start", "/api/start")}>
                        Start Server
                    </LuxuryButton>
                )}
            </div>
            ...
            <div className="flex flex-col gap-6 w-full max-w-[200px] text-center md:text-left">
                <LuxuryButton variant="text" onClick={() => handleAction("Restore", "/api/restore")} disabled={status !== "running"}>
                    Restore
                </LuxuryButton>
                 <LuxuryButton variant="text" onClick={() => {/* Open Settings */}}>
                    Settings
                </LuxuryButton>
            </div>
