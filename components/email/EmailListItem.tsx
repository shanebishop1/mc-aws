...
export const EmailListItem = ({ email, onRemove, disabled }: EmailListItemProps) => {
  return (
    ...
    >
      <span className="min-w-0 break-all font-sans text-sm text-charcoal">{email}</span>
      <button
        type="button"
        onClick={() => onRemove(email)}
        disabled={disabled}
        aria-label={`Remove ${email}`}
        className="ml-2 shrink-0 text-charcoal/40 hover:text-red-500 transition-colors disabled:opacity-50"
      >
