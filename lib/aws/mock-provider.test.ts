...
describe("Mock Provider Core", () => {
  ...
  describe("Server Action Lock", () => {
    it("should acquire and release action lock", async () => {
      let lockAcquired = false;

      await mockProvider.withServerActionLock("test-action", async () => {
        lockAcquired = true;
        // Check that action is set
        const action = await mockProvider.getServerAction();
    ...
    it("should prevent concurrent actions", async () => {
      // Set an action lock
      await mockProvider.setServerAction("existing-action");

      // Try to acquire another action lock
      await expect(
        mockProvider.withServerActionLock("new-action", async () => {
          // This should not execute
        })
      ).rejects.toThrow("Cannot start new-action. Another operation is in progress: existing-action");
    });

    it("should clear stale action locks", async () => {
      const stateStore = getMockStateStore();

      // Set an action with an old timestamp (more than 30 minutes ago)
      ...
      // Should clear the stale lock
      const action = await mockProvider.getServerAction();
      expect(action).toBeNull();
    });

    it("should release lock even when action fails", async () => {
      try {
        await mockProvider.withServerActionLock("failing-action", async () => {
    ...
    it("should set and get server action", async () => {
      await mockProvider.setServerAction("backup-operation");

      const action = await mockProvider.getServerAction();
      expect(action).not.toBeNull();
      expect(action?.action).toBe("backup-operation");
      expect(action?.timestamp).toBeDefined();
    });

    it("should delete server action", async () => {
      await mockProvider.setServerAction("test-action");
      expect(await mockProvider.getServerAction()).not.toBeNull();

      await mockProvider.deleteParameter("/minecraft/server-action");
      expect(await mockProvider.getServerAction()).toBeNull();
    });
  });
