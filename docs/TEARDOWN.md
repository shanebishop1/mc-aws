...
8. only after the instance is stopped, creates, waits for, tag-verifies, records, and reports a final snapshot of the managed root volume;
9. when hibernated with no root volume, instead parses the real `/minecraft/backups-cache` shape `{backups:[...], cachedAt:<epoch-ms>}`, requires non-empty backups plus a valid positive `cachedAt`, and records that evidence;
...
CloudFormation normally deletes the instance's root EBS volume. **The root volume itself does not survive stack deletion.** To preserve application-consistent final contents without changing the normal block-device lifecycle or causing replacement during upgrades, execution never snapshots a running instance. For `running`/`pending`, it sends a bounded graceful `systemctl stop minecraft.service` command through SSM, verifies exact `Success`, stops the EC2 instance, waits for `stopped`, and then re-reads the exact instance, root mapping, attachment, and ownership tags. An already stopped instance proceeds directly; an instance already stopping must finish stopping. Any quiesce, stop, wait, identity, or tag failure blocks deletion.

Execution then creates a final `McAwsFinalTeardown=true` snapshot, records it as the pending manifest attempt, waits for `completed`, verifies exact source/stack tags, and updates the manifest's final-snapshot pointer. Stack deletion is blocked if creation, waiting, or verification fails.
...
When hibernate has already removed the root volume, no EBS snapshot can be created. Teardown requires the handler's exact cache object with a non-empty `backups` array and valid `cachedAt` epoch timestamp. This is only evidence that the application previously cached a non-empty listing; it is **not** live external Google Drive verification and is not a substitute for checking Google Drive or performing a restore test.
...
7. **Final root data:** if a tagged managed root volume is attached, gracefully stop Minecraft through SSM when running, verify SSM success, stop/wait for the instance, re-read exact identities/tags, then create and verify the tagged final snapshot. Never snapshot a running instance. If no root exists, verify Google Drive separately and parse the non-empty `{backups,cachedAt}` cache only as supporting evidence. Do not continue on failure.
