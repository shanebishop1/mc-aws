...
- Excluded from staging: `.agents/`, `AGENTS.md`, `CLAUDE.md`, and zero-byte `3`. No `__pycache__` artifacts were
  present at handoff, so no generated bytecode was removed. Ignored local artifacts, real secrets, deployment-local
